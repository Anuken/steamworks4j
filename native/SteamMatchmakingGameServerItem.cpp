...
void convertGameServerItem(jobject item, JNIEnv* env, const gameserveritem_t& server) {
	jclass ex = env->FindClass("steamworks/SteamException");
	jclass itemClazz = env->FindClass("steamworks/SteamMatchmakingGameServerItem");

	jfieldID field = env->GetFieldID(itemClazz, "netAdr", "Lsteamworks/SteamMatchmakingServerNetAdr;");
	jclass netAdrClazz = env->FindClass("steamworks/SteamMatchmakingServerNetAdr");
